...
cv::Mat preprocess_img(cv::Mat& img, int input_w, int input_h)
{
    // resize
    cv::Mat resize_img;
    cv::resize(img, resize_img, cv::Size(int(input_w * 1.143), int(input_h * 1.143)));
    // center crop
    int crop_top = (resize_img.rows - input_h) / 2;
    int crop_left = (resize_img.cols - input_w) / 2;
    cv::Mat cropped_img(resize_img, cv::Range(crop_top, crop_top + input_h), cv::Range(crop_left, crop_left + input_w));
    cv::Mat cropped;
    cropped = cropped_img.clone();
